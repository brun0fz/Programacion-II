...
nodoArbol* insertarArbolOrdenado(nodoArbol *arbolito, persona dato)
{
    ...
    {
        if(dato.legajo > arbolito->dato.legajo) //si el dato es mayor, se va por la derecha
        {
            arbolito->der=insertarArbolOrdenado(arbolito->der, dato);
        }
        else if(dato.legajo < arbolito->dato.legajo) //si el dato es menor se va por izquierda
        {
...
int alturaDelArbol(nodoArbol* arbol)
{
    ...
    else
    {
        ...
        {
            resp=1 + mayor(alturaDelArbol(arbol->izquierda), alturaDelArbol(arbol->derecha));
        }
    }
    return resp;
}
...
nodoArbol *borrarNodoArbol(nodoArbol *arbol, int dato)
{
    if(arbol)
    {
        if (dato == arbol->dato.legajo)//en caso de que sea igual
        {
            if(arbol->izq)
            {
                nodoArbol *masDer=nodoMasDerecha(arbol->izq);
                arbol->dato=masDer->dato;
                arbol->izq=borrarNodoArbol(arbol->izq, masDer->dato.legajo);
            }
            else if(arbol->der)
            {
                nodoArbol *masIzq=nodoMasIzquierda(arbol->der);
                arbol->dato=masIzq->dato;
                arbol->der=borrarNodoArbol(arbol->der, masIzq->dato.legajo);
            }
